...
while ($term = $selectterms->fetch_object()) {
    ...
    if ($selectdefinitions = $mysqli->query('select  definition_id ,definition, first_name, email ,  updated from definition , student where student_email = email and term_term="'.$term->term.'" order by definition_id desc')) {
