			...
			if (lesson.exercises) {
				...
				$.each(lesson.exercises, function(index, value) {
					...
					if (lesson.exercises[index].url) {
						lessoncontent += '<a href="'+lesson.exercises[index].url+'">'+lesson.exercises[index].description+'</a></p>';
					} else {
						lessoncontent += lesson.exercises[index].description+'</p>';
					}
					...
					if (lesson.exercises[index].homeworks) {
						$.each(lesson.exercises[index].homeworks,function(index2,value2) {
							lessoncontent += '<p class="muted"><a data-reviewee='+ lesson.exercises[index].homeworks[index2].first_name+' data-homeworkid="'+ index2+'" class="btn btn-mini thumbs thumbsup" href="#write-review"><i class="icon-thumbs-up"></i></a><a data-reviewee='+ lesson.exercises[index].homeworks[index2].first_name+'  data-homeworkid="'+ index2 +'"  class="btn btn-mini thumbs thumbsdown" href="#write-review"><i class="icon-thumbs-down"></i></a>&nbsp;&nbsp;';
			...
			$(".submit-hw").click(function() {
				...
				switch (lesson.exercises[id].type) {
					case "url":
						...
						break;
					case "form":
						$.each(lesson.exercises[id].questions,function(index2, value2) {
							exercisecontent += '<p class="span5">'+value2.text+'</p><p class="span5"><textarea columns=600 rows=2 class="span5" id="exercise-q'+index2+'" name="exercise-q'+index2+'"></textarea></p>';
						});
						break;
				}
